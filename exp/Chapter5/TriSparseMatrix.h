...
class TriSparseMatrix
{
    ...
    TriSparseMatrix<ElemType> operator+(const TriSparseMatrix<ElemType>& rhs); // 重载加法运算符
};
...
{
    TriSparseMatrix<ElemType> ans(rows, cols);
    if (rows != rhs.rows || cols != rhs.cols) throw Error("矩阵的大小不同");
    int i = 0, j = 0;
    ans.num = 0;
    while (i < num && j < rhs.num)
    {
        // 行号小
        if (triElems[i].row < rhs.triElems[j].row)
            ans.triElems[ans.num++] = {triElems[i].row, triElems[i].col, triElems[i].value}, i++;
        // 行号大
        else if (triElems[i].row > rhs.triElems[j].row)
            ans.triElems[ans.num++] = {rhs.triElems[j].row, rhs.triElems[j].col, rhs.triElems[j].value}, j++;
        // 同行
        else
        {
            // 列号小
            if (triElems[i].col < rhs.triElems[j].col)
                ans.triElems[ans.num++] = {triElems[i].row, triElems[i].col, triElems[i].value}, i++;
            // 列号大
            else if (triElems[i].col > rhs.triElems[j].col)
                ans.triElems[ans.num++] = {rhs.triElems[j].row, rhs.triElems[j].col, rhs.triElems[j].value}, j++;
            // 同列
            else
            {
                int tmp = triElems[i].value + rhs.triElems[j].value;
                if (tmp) ans.triElems[ans.num++] = {triElems[i].row, triElems[i].col, tmp};
                i++, j++;
            }
        }
    }
    // 剩余元素
    while (i < num) ans.triElems[ans.num++] = {triElems[i].row, triElems[i].col, triElems[i].value}, i++;
    while (j < rhs.num) ans.triElems[ans.num++] = {rhs.triElems[j].row, rhs.triElems[j].col, triElems[j].value}, j++;
    return ans;
}
