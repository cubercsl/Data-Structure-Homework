...
class String
{
    ...
    int Find(const String& pat, int pos);              // 模式匹配(1)
    String& Replace(const String& t, const String& v); // 字符串替换(1)
};

// 串相关操作
void Write(const String& s); // 输出串
void Read(String& s);        // 输入串
...
// 操作结果：输入串
{
    string s;
    cin >> s;
    char* cs = new char[s.length() + 1]; // 申请临时空间
...
int String::Find(const String& pat, int pos = 0)
{
    int i = pos, j = 0;
    for (; i < length && j < pat.length; sVal[i] == pat.sVal[j] ? (i++, j++) : (i -= (j - 1), j = 0))
        ;
...
String& String::Replace(const String& t, const String& v)
{
    ...
    while (~(pos = Find(t, k)))
    {
        for (int i = k; i < pos; i++) s.push_back(sVal[i]);
        for (int i = 0; i < v.length; i++) s.push_back(v.sVal[i]);
        k = pos + t.length;
    }
    for (int i = k; i < length; i++) s.push_back(sVal[i]);
    length = s.size();
    delete[] sVal;
    sVal = new char[length + 1];
    for (int i = 0; i < length; i++) sVal[i] = s[i];
    s[length] = '\0';
    return *this;
}
#endif
