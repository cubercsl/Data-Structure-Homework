...
class GenList
{
    ...
    int CountHelp(const GenListNode<ElemType>* hd);
    // 统计以hd为表头的广义表的原子数目(10)
...
template <class ElemType>
int GenList<ElemType>::CountHelp(const GenListNode<ElemType>* hd)
{
    int count = 0;
    for (auto* p = hd->tLink; p != NULL; p = p->tLink)
    {
        if (p->tag == LIST)
            count += CountHelp(p->hLink);
        else
            count++;
    }
    return count;
...
template <class ElemType>
int GenList<ElemType>::CountAtom()
{
    return CountHelp(head);
}
