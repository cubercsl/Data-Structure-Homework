...
public:
    ...
    DblNode<ElemType>* Search(ElemType key);                           // 查找元素
};
...
template <class ElemType>
DblNode<ElemType>* DblLinkList::Search(ElemType key)
{
    DblNode<ElemType>* p;
    if (head == NULL || key < head->data || key > head->prior->data) return NULL;
    if (key < sp->data)
    {
        p = sp->prior;
        while (p->data > key) p = p->prior;
    }
    else
    {
        p = sp->next;
