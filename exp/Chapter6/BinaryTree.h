...
class BinaryTree
{
    ...
    // 赋值运算符重载
    int CountWidth() const; // 计算二叉树最大宽度(9)
    int CountLeaf() const;  // 计算二叉树叶子结点数量(10)
    int CountLeaf(const BinTreeNode<ElemType>* r) const;
};
...
// 操作结果：中序遍历以r为根的二叉树
{
    ...
    // }
    std::stack<BinTreeNode<ElemType>*> s;
...
int BinaryTree<ElemType>::CountWidth() const
{
    ...
    while (!q.IsEmpty())
    {
        ...
        {
            q.DelQueue(p);
            if (p->leftChild != NULL) q.EnQueue(p->leftChild), m++;
            if (p->rightChild != NULL) q.EnQueue(p->rightChild), m++;
        }
        n = m;
...
template <class ElemType>
int BinaryTree<ElemType>::CountLeaf() const { return CountLeaf(root); }
