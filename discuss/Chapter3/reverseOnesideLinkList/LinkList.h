...
//  单链表的函数成员
	...
	//实现逆置
    Node<ElemType> *recursion(Node<ElemType> *p);//递归函数本体
	void reverseFunc1();//递归实现逆置
	void reverseFunc2();//循环实现逆置
};
...
Node<ElemType> *LinkList<ElemType>::recursion(Node<ElemType> *p)
//返回尾结点指针，参数为本层结点指针
{
    if(p->next == NULL) return p;               //递归边界：到达链表尾端，返回尾端指针
    Node<ElemType> *q = recursion(p->next);     //未到达边界继续递归
    p->next->next = p;                          //使本结点的后继next指向本层地址
    p->next = NULL;                             //设本层结点为头结点后第一个结点指向NULL，若不是，会在本层回溯后在上一层递归中再指向其前驱
    return q;                                   //返回的实际上就是尾结点指针
}

template <typename ElemType>
void LinkList<ElemType>::reverseFunc1()
// 操作结果：链表逆置，递归写法
{
    Node<ElemType> *p=head->next;               //从头结点后第一个结点开始
    if(p == NULL) return;                       //空表
    head->next = recursion(p);                  //头结点指向尾结点
}

template <typename ElemType>
void LinkList<ElemType>::reverseFunc2()
// 操作结果：链表逆置，循环过程把所有next指针逆指
// 时间复杂度O(n),空间复杂度O(1)
{
    Node<ElemType> *p=head->next;   //遍历指针
    Node<ElemType> *temp,*nextTemp; //temp保存前驱地址，nextTemp暂存后继地址
    for(;p->next!=NULL;p=nextTemp)
    {
        if(p==head->next)
        {
            nextTemp=p->next;       //在修改next前先将后继的地址暂存起来
            p->next=NULL;           //第一个结点逆置后指向空
        }
        else
        {
            nextTemp=p->next;
            p->next=temp;           //将本结点next指针指向上一次循环时存下来的前一个结点地址，即本结点的前驱
        }
        temp=p;
    }
    head->next=p;
